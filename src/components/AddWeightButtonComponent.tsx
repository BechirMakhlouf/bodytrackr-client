...
export default function AddWeightButton() {
  ...
  const { state: weightLog, setState: setWeightLog } = useContext(
    weightLogContext,
  );
  ...
  const onWeightSubmit: SubmitHandler<Weight> = (submittedWeight: Weight) => {
    ...
    if (dateIndexInWeightLog !== -1) {
      ...
      setWeightLog((prevWeightLog) => [...prevWeightLog]);
      return;
    }
  ...
  return (
    <motion.div className="fixed bottom-4 custom-shadow right-8 h-12 rounded-full
